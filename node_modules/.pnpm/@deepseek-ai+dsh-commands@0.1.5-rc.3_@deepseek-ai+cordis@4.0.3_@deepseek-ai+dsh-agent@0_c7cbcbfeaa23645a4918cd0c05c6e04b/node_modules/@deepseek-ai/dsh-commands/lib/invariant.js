...
const install = Object.assign((ctx, fail) => {
	...
	const validateEvent = (session, event) => {
		...
		const sourceEvent = source === void 0 ? void 0 : session.eventAt(source);
	...
	for (const session of ctx.sessions.list()) for (const event of session.snapshotEvents()) validateEvent(session, event);
